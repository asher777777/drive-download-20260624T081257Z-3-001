...
export async function generatePageWithAI(
  ...
  try {
    ...
    const { getCompanyServices } = await import("@/features/company-services/actions");
    
    const [globalSettings, companyServices] = await Promise.all([
      getGlobalSettings(),
      getCompanyServices()
    ]);

    const companyInfoContext = `
Company Name: ${globalSettings?.companyName || 'Not specified'}
Company Vision: ${globalSettings?.companyVision || 'Not specified'}
Company Slogan: ${globalSettings?.slogan || 'Not specified'}
Company Services Summary: ${companyServices?.map(s => s.name).join(', ') || 'Not specified'}
`;
    ...
    let systemPrompt = `You are an expert copywriter and web designer for the number one digital systems and software company. Your goal is to produce extremely high-quality, engaging, professional, and convincing Hebrew content for a page.
    
BRANDING CONTEXT (Must be reflected in the content):
${companyInfoContext}
...
4. IMAGE PROMPT IS MANDATORY: You MUST ALWAYS generate a detailed English "imagePrompt" in the root of the JSON for a professional cover image, even if you are told not to generate images. This prompt will be saved for later use.
...
You MUST provide a "sectionOrder" array containing exactly these section keys: "hero", "mainContent", "services", "community", "livePosts", "contact", "landingSection", "richContent", "pricing", "timer", "videoGallery", "imageListing", "testimonials", "courseBanner".
...
JSON Structure Example (Adapt to include all allowed sections properly):
{
  ...
  "videoGallery": { "visible": false, "title": "..." },
  "imageListing": { "visible": false, "title": "..." },
  "testimonials": { "visible": false, "title": "..." },
  "courseBanner": { "visible": false, "title": "..." },
  "sectionOrder": ["hero", "richContent", "services", "contact", "mainContent", "community", "landingSection", "livePosts", "pricing", "timer", "videoGallery", "imageListing", "testimonials", "courseBanner"],
...
CRITICAL: You MUST include a "slug" field in the JSON with a short, highly focused English URL slug translated from the page's main topic. You MUST ALSO fill the "seo" object intelligently in Hebrew.`;
    ...
    // Explicitly keep the imagePrompt inside generatedData so savePageConfig stores it.
    if (imagePrompt && !generatedData.imagePrompt) {
      generatedData.imagePrompt = imagePrompt;
    }
...
export async function generateServiceWithAI(prompt: string, slug: string, generateImages: boolean = true) {
  return generatePageWithAI(prompt, slug, 'service', 'רגיל', 'כולם', ['hero', 'services', 'contact'], '', '', generateImages);
}
