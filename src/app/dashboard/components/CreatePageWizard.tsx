  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const collectionAudiences = audiencesData.map(a => a.name);
  ...
  const handleGenerate = async () => {
    ...
    try {
      if (audience === "other" && customAudience) {
        ...
        await addAudience(customAudience).catch(err => console.warn("Failed to save custom audience", err));
      }
              ...
              <div className="grid grid-cols-2 gap-3">
                {TONES.map(t => (
                  <button
                    key={t}
                    onClick={() => setTone(t)}
                    className={`p-4 rounded-2xl border transition-all duration-200 text-right ${
                      tone === t 
